...
`c1.asm` is the first production entry under the replacement `src/` area. It
reuses the preserved ATOM reader, numeric ABI and publication modules while the
compact compiler is built in measured vertical slices. The include boundary is
...
boundaries for this first compiler slice, not reduced language semantics.
