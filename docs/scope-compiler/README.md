...
`write-char`, `read-char`, package definitions, lexical `let` and `let*`, `if`,
`begin`, `and`, `or`, `+`,
...
The compiler image is loaded at `$0100`. The current image is 17,512 bytes
origin-relative, which is 1,128 bytes above the 16 KiB code reference.
...
are available and the current account uses 50,152 bytes.

The runtime provider is currently 6,633 bytes origin-relative. The fixed tables
