...
function colorSequence(
  ...
  if (
    typeof color !== "string" ||
    !Object.prototype.hasOwnProperty.call(table, color)
  ) {
    throw new RangeError(`Unknown terminal color: ${String(color)}`);
  }
  const sequence = table[color];
  if (typeof sequence !== "string") {
    throw new RangeError(`Unknown terminal color: ${color}`);
  }
  return sequence;
}
...
export function createTerminalLibrary(client: EffectClient): TerminalLibrary {
  return {
    ...
    foreground: (color) =>
      control(client, colorSequence(ANSI_FOREGROUND, color)),
    background: (color) =>
      control(client, colorSequence(ANSI_BACKGROUND, color)),
