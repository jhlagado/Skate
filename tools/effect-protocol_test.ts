...
Deno.test("version-one text request keeps the published wire vector", () => {
  assert.deepEqual(EffectFrameKindCode, { request: 1, response: 2, event: 3 });
  const encoded = encodeEffectCommand(
  ...
  assert.deepEqual(
    decodeEffectCommand(decodeEffectFrame(encoded)),
    { type: "text", bytes: Uint8Array.of(0x41) },
  );
});

Deno.test("extracted validation keeps established error codes", () => {
  assert.throws(
    () =>
      encodeEffectCommand(
        { type: "text", bytes: "A" as unknown as Uint8Array },
        1,
      ),
    (error) =>
  ...
  assert.throws(
    () => client.sendText(42 as unknown as string),
    (error) =>
...
Deno.test("effect framing rejects malformed, truncated, corrupt and oversized input", () => {
  ...
  const partial = new EffectFrameDecoder();
  partial.push(frameBytes().slice(0, -1));
  assert.throws(
    () => partial.finish(),
    (error) =>
      error instanceof EffectProtocolError && error.code === "truncated",
